...
class MultiheadAttention(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        # for att calibration
        self.calibration_mode = 'what?'
        self.confidence=None

        self.scaling_factor_for_att_hidden_temp = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
        self.scaling_factor_for_mh_att_hidden_temp = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
        self.scaling_factor_for_ad_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
        ...
        if conf_calibration:
            ...
            self.scaling_factor_for_conf_att_hidden_temp2 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
            self.scaling_factor_for_conf_mh_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
            self.scaling_factor_for_conf_mh_att_hidden_temp2 = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
            self.scaling_factor_for_conf_ad_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
            self.scaling_factor_for_conf_ad_att_hidden_temp2 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
            self.scaling_factor_for_conf_ad_att_hidden_temp3 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
            self.scaling_factor_for_conf_mh_ad_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
            self.scaling_factor_for_conf_mh_ad_att_hidden_temp2 = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
            self.scaling_factor_for_conf_mh_ad_att_hidden_temp3 = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
        else:
            self.scaling_factor_for_conf_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
            self.scaling_factor_for_conf_mh_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(num_heads), requires_grad=True)
            self.scaling_factor_for_conf_ad_att_hidden_temp1 = torch.nn.Parameter(torch.zeros(1), requires_grad=True)
    ...
    def forward(
        ...
        if self.calibration_mode == 'att_temp':
            ...
            attn_weights = attn_weights * (1+self.scaling_factor_for_ad_att_hidden_temp1 * self.lr_regularizer + entropy_att_weight * self.scaling_factor_for_ad_att_hidden_temp2 * self.lr_regularizer)
            ...
            if self.conf_calibration:
                ...
                attn_weights = attn_weights * (1 + self.scaling_factor_for_conf_att_hidden_temp1 * self.lr_regularizer)
        elif self.calibration_mode == 'mh_att_temp_conf':
            if self.conf_calibration:
                attn_weights = attn_weights * (1 + self.scaling_factor_for_conf_mh_att_hidden_temp1.view(1,self.num_heads,1,1) * self.lr_regularizer + self.scaling_factor_for_conf_mh_att_hidden_temp2.view(1,self.num_heads,1,1) * self.lr_regularizer * self.confidence)
            else:
                attn_weights = attn_weights * (1 + self.scaling_factor_for_conf_mh_att_hidden_temp1.view(1,self.num_heads,1,1) * self.lr_regularizer)
        elif self.calibration_mode == 'ad_att_temp_conf':
            entropy_att_weight = (torch.softmax(attn_weights.detach(), dim=-1) * torch.log_softmax(attn_weights.detach(), dim=-1)).sum(-1).unsqueeze(-1) # B,H,T1,T2 -> B,H,T1,1
            if self.conf_calibration:
                attn_weights = attn_weights * (1+self.scaling_factor_for_conf_ad_att_hidden_temp1 * self.lr_regularizer + entropy_att_weight * self.scaling_factor_for_conf_ad_att_hidden_temp2 * self.lr_regularizer + self.confidence * self.scaling_factor_for_conf_ad_att_hidden_temp3 * self.lr_regularizer)
            else:
                attn_weights = attn_weights * (1+self.scaling_factor_for_conf_ad_att_hidden_temp1 * self.lr_regularizer + entropy_att_weight * self.scaling_factor_for_conf_ad_att_hidden_temp2 * self.lr_regularizer)
        elif self.calibration_mode == 'mh_ad_att_temp_conf':
            entropy_att_weight = (torch.softmax(attn_weights.detach(), dim=-1) * torch.log_softmax(attn_weights.detach(), dim=-1)).sum(-1).unsqueeze(-1) # B,H,T1,T2 -> B,H,T1,1
            if self.conf_calibration:
        ...
        if self.encoder_decoder_attention and bsz != kv_bsz:
            ...
            attn = torch.bmm(attn_probs.view(bsz * self.num_heads, tgt_len, src_len), v)
    ...
    def turn_calibration_mode(self, att_cali):
        self.calibration_mode = att_cali
