...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        # for conf-calibration
        confidence = (
            torch.ones(bsz * beam_size, max_len + 2)
            .to(src_tokens)
            .float()
        )  # +2 for eos and pad
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if len(finalized_sents) > 0:
                ...
                confidence = confidence.view(bsz, -1)[batch_idxs].view(new_bsz * beam_size, -1) # added for conf-calibration
                lprobs_backup = lprobs_backup.view(bsz, -1, lprobs_backup.size(-1))[batch_idxs].view(new_bsz * beam_size, lprobs_backup.size(-1))
                if attn is not None:
            ...
            confidence[:, : step + 1] = torch.index_select(
                confidence[:, : step + 1], dim=0, index=active_bbsz_idx
            )
            lprobs_backup = torch.index_select(
                lprobs_backup, dim=0, index=active_bbsz_idx
            )
            # Select the next token for each of them
            tokens.view(bsz, beam_size, -1)[:, :, step + 1] = torch.gather(
                cand_indices, dim=1, index=active_hypos
            )
            # update confidence ver2.0
            # lprobs: torch.Size([1280, 6632])
            tokens_one_hot = torch.nn.functional.one_hot(tokens[:,step+1], num_classes=lprobs_backup.size(-1)) # 
            conf = (lprobs_backup*tokens_one_hot).max(-1).values
            confidence[:,step+1] = conf
...
class EnsembleModel(nn.Module):
    ...
    def turn_calibration_mode(self, calibration_func):
        self.calibration_mode = calibration_func
        for model in self.models:
            for i in model.encoder.layers:
                i.turn_calibration_mode_encoder(calibration_func)
            for i in model.decoder.layers:
                i.turn_calibration_mode_decoder(calibration_func)
            model.decoder.turn_calibration_mode_decoder_temp(calibration_func)
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            ...
            # set confidence for decoder
            if self.calibration_mode is not None and '_conf' in self.calibration_mode:
                for layer_dec in model.decoder.layers:
                    layer_dec.set_confidence_decoder(confidence)
                model.decoder.set_confidence_decoder_temp(confidence)
