...
class FairseqTask(object):
    ...
    def train_step(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False, sample_valid=None
    ):
        """
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                if sample_valid is not None:
                    loss, sample_size, logging_output = criterion(model, sample, optimizer, sample_valid, update_num=update_num)
                else:
                    loss, sample_size, logging_output = criterion(model, sample, optimizer, update_num=update_num)
    ...
    def valid_step(self, sample, model, criterion):
        ...
        with torch.no_grad():
            # loss, sample_size, logging_output = criterion(model, sample)
            loss, sample_size, logging_output = criterion(model, sample, stage_validation=True)
        return loss, sample_size, logging_output
